                        ...
                        !downState && (
                          <div>
                            <div className="flex items-center">
                              <span className="border border-stone-500 rounded-s-md py-2 px-4">Quantity</span>
                              <input onChange={(e) => setQuantity(e.target.value)} type="number" className="bg-transparent ps-3 rounded-e-md focus:border-blue-500 outline-none w-full border-2 py-2" defaultValue={multidata.stock} placeholder="350" />
                            </div>
                            <button className="bg-blue-600 hover:bg-blue-500 w-full mt-4 py-2 rounded-md">Check out for {quantity > 1 ? quantity * multidata.price : multidata.price} </button>
                          </div>
                        )
                      }
                    </div>
                ...
                <div>
                  {
                    modatData?.cryptoData?.length > 1 ? (
                      ...
                      :
                      <div>
                        <p className="text-xl text-center">{modatData.cryptoTitle}</p>
                        <div className="text-center text-sm flex justify-center">
                          <span>min. Order:1/</span>
                          <p>
                            Stock: {modatData?.cryptoData?.stock} /
                          </p>
                          <p>
                            Price:{modatData?.cryptoData?.price}
                          </p>
                        </div>
                        ...
                        <div>
                          {/* header text */}
                          {
                            ...
                            (
                              <div>
                                ...
                                <div >
                                  {
                                    modatData?.cryptoData?.discription.whayNeed.map(item => <li className="py-2 text-lg">{item}</li>)
                                  }
                                </div>
                              </div>
                            )
                          ...
                          {
                            modatData?.cryptoData?.discription.headerText && <p className="py-4 text-xl">{modatData?.cryptoData?.discription.headerText}</p>
                          }
                          {
                            modatData?.cryptoData?.discription.comes_with && <p className="text-xl my-4">{modatData?.cryptoData?.discription.comes_with}</p>
                          }
                          {
                            modatData?.cryptoData?.discription.middelText && <p className="text-xl my-4">{modatData?.cryptoData?.discription.middelText}</p>
                          }
                          {
                            modatData?.cryptoData?.discription.country && <p className="text-xl my-4"> Country:{modatData?.cryptoData?.discription.country}</p>
                          }
                          {
                            modatData?.cryptoData?.discription.afterPurchase && (
                              <div>
                                <p className="text-xl my-4">After Purchase what do I do?</p>
                                {
                                  modatData?.cryptoData?.discription.afterPurchase.map(item => <li className="text-lg my-4">{item}</li>)
                                }
                              </div>
                            )
                          ...
                          {
                            modatData?.cryptoData?.discription.YouGet && <p className="text-xl my-4">you will get: {modatData?.cryptoData?.discription.YouGet}</p>
                          }
                          {
                            modatData?.cryptoData?.discription.YouNotGet && <p className="text-xl my-4">What you will not get{modatData?.cryptoData?.discription.YouNotGet}</p>
                          }
                          {
                            modatData?.cryptoData?.discription.specialNote && <p className="text-xl my-4">Special Note :{modatData?.cryptoData?.discription.specialNote}</p>
                          }
                          {
                            modatData?.cryptoData?.discription.doesntComeWith && <p className="text-xl my-4">Doesn’t Come with : {modatData?.cryptoData?.discription.doesntComeWith}</p>
                          }
                          {
                            modatData?.cryptoData?.discription.footerText && <p className="text-xl my-4">What you will not get{modatData?.cryptoData?.discription.footerText}</p>
                          }
                          {/* -- end */}
                          {
                            modatData?.cryptoData?.discription.telegramLink && (
                              <div>
                                {
                                  modatData?.cryptoData?.discription.telegramLink.map(item => <div className="text-xl py-4">
                                    <p>🔔Get FREE Access To Our Secret Channel:</p>
                                    <a className="text-white hover:text-blue-300" href={item}>{item}</a>
